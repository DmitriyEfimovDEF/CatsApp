...
object YFastAdapterDiffUtil {
    ...
    private fun <A : ModelAdapter<Model, Item>, Model, Item : GenericItem> setItems(
        adapter: A,
        items: List<Item>
    ): A {
    ...
    fun <Item : GenericItem> calculateDiff(
        oldItems: List<Item>,
        newItems: List<Item>,
        ...
        return DiffUtil.calculateDiff(
            FastAdapterCallback(oldItems, newItems, callback),
            detectMoves
        )
    }
    ...
    private fun <Item : GenericItem> collapseIfPossible(fastAdapter: FastAdapter<Item>?) {
        fastAdapter ?: return
        try {
            val c: Class<IAdapterExtension<Item>> =
                Class.forName("com.mikepenz.fastadapter.expandable.ExpandableExtension") as Class<IAdapterExtension<Item>>
     ...
     */
    operator fun <A : ModelAdapter<Model, Item>, Model, Item : GenericItem> set(
        adapter: A,
     ...
     */
    private class FastAdapterCallback<Item : GenericItem> internal constructor(
        private val oldItems: List<Item>,
        private val newItems: List<Item>,
        private val callback: DiffCallback<Item>
    ) : DiffUtil.Callback() {
        ...
        override fun getChangePayload(oldItemPosition: Int, newItemPosition: Int): Any? {
            val result = callback.getChangePayload(
     ...
     */
    private class FastAdapterListUpdateCallback<A : ModelAdapter<Model, Item>, Model, Item : GenericItem> internal constructor(
        ...
        override fun onMoved(fromPosition: Int, toPosition: Int) {
            adapter.fastAdapter?.notifyAdapterItemMoved(
                preItemCountByOrder + fromPosition,
                toPosition
            )
        }
